...
document.addEventListener('DOMContentLoaded', () => {
        ...
        sucursalModalContainer.innerHTML = `
            <div class="bg-[#1e293b] rounded-lg shadow-xl w-full max-w-lg">
                <div class="p-6 border-b border-gray-700 flex justify-between items-center">
                    <h2 class="text-2xl font-bold text-white">${isEditing ? 'Editar Sucursal' : 'Nueva Sucursal'}</h2>
                ...
                <form id="sucursal-form" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nombre" required class="mt-1 block w-full bg-gray-700 rounded-md p-2 border border-gray-600" value="${sucursal?.nombre || ''}">
                    </div>
                    <div>
                        <label for="direccion" class="block text-sm font-medium">Dirección</label>
                        <input type="text" name="direccion" required class="mt-1 block w-full bg-gray-700 rounded-md p-2 border border-gray-600" value="${sucursal?.direccion || ''}">
                    </div>
                    <div>
                        <label for="telefono" class="block text-sm font-medium">Teléfono</label>
                        <input type="tel" name="telefono" class="mt-1 block w-full bg-gray-700 rounded-md p-2 border border-gray-600" value="${sucursal?.telefono || ''}">
                    </div>
                    <div>
                        <label for="email" class="block text-sm font-medium">Email</label>
                        <input type="email" name="email" class="mt-1 block w-full bg-gray-700 rounded-md p-2 border border-gray-600" value="${sucursal?.email || ''}">
                    </div>
                    <div class="flex justify-end space-x-4 pt-4">
                        <button type="button" class="close-modal-btn bg-gray-600 hover:bg-gray-500 font-bold py-2 px-4 rounded-lg">Cancelar</button>
                        <button type="submit" class="bg-green-600 hover:bg-green-500 font-bold py-2 px-6 rounded-lg">Guardar</button>
                    </div>
                </form>
            </div>`;
    ...
    const renderSucursales = (sucursales) => {
        ...
        if (sucursales.length === 0) {
            sucursalesTableBody.innerHTML = `<tr><td colspan="4" class="text-center py-10 text-gray-500">No hay sucursales registradas.</td></tr>`;
            return;
        }
        sucursales.forEach(s => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">${s.nombre}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">${s.direccion}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">${s.telefono || 'N/A'}</td>
    ...
    const renderUsuarioModal = (usuario = null) => {
        ...
        usuarioModalContainer.innerHTML = `
            <div class="bg-[#1e293b] rounded-lg shadow-xl w-full max-w-lg">
                <div class="p-6 border-b border-gray-700 flex justify-between items-center">
                    <h2 class="text-2xl font-bold text-white">${isEditing ? 'Editar Usuario' : 'Nuevo Usuario'}</h2>
                    <button class="close-modal-btn text-gray-400 hover:text-white text-2xl">&times;</button>
                </div>
                <form id="usuario-form" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nombre" required class="mt-1 block w-full bg-gray-700 rounded-md p-2 border border-gray-600" value="${usuario?.nombre || ''}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Username</label>
                        <input type="text" name="username" required class="mt-1 block w-full bg-gray-700 rounded-md p-2 border border-gray-600" value="${usuario?.username || ''}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Contraseña</label>
                        <input type="password" name="password" class="mt-1 block w-full bg-gray-700 rounded-md p-2 border border-gray-600" placeholder="${isEditing ? 'Dejar en blanco para no cambiar' : ''}" ${!isEditing ? 'required' : ''}>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="id_sucursal" required class="mt-1 block w-full bg-gray-700 rounded-md p-2 border border-gray-600">${sucursalOptions}</select>
                        </div>
                    </div>
                    <div class="flex justify-end space-x-4 pt-4">
                        <button type="button" class="close-modal-btn bg-gray-600 hover:bg-gray-500 font-bold py-2 px-4 rounded-lg">Cancelar</button>
    ...
    const renderUsuarios = (usuarios) => {
        ...
        if (usuarios.length === 0) {
            usuariosTableBody.innerHTML = `<tr><td colspan="5" class="text-center py-10 text-gray-500">No hay usuarios registrados.</td></tr>`;
            return;
        }
        usuarios.forEach(u => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">${u.nombre}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">${u.username}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">${u.rol}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">${u.sucursal_nombre || 'N/A'}</td>
                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
